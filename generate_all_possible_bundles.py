...
def read_products(ws):
    ...
    for row in range(2, MAX_PRODUCTS + 2):
        ...
        avg_qty_4m = sold_qty_4m / 4 if sold_qty_4m > 0 else 0.0

        stock = to_number(ws[f"J{row}"].value)
        reserved_stock = to_number(ws[f"K{row}"].value)
        available_stock = to_number(ws[f"L{row}"].value, stock - reserved_stock)
        ...
        products.append(
            {
                ...
                "purchased_item_count": to_number(ws[f"D{row}"].value),
                "sold_qty_last_4m": sold_qty_4m,
                "avg_qty_4m": avg_qty_4m,
                "contribution": normalize_percent(ws[f"I{row}"].value),
                "stock": stock,
...
def score_products(products):
    sold_values = [p["sold_qty_last_4m"] for p in products]
    contribution_values = [p["contribution"] for p in products]
    coverage_values = []
    for product in products:
        avg = product["avg_qty_4m"]
        coverage_values.append(99.0 if avg == 0 else product["available_stock"] / avg)

    for idx, product in enumerate(products):
        movement_percentile = percent_rank_inc(sold_values, product["sold_qty_last_4m"])
        slow_mover_score = 1.0 - movement_percentile
        contribution_percentile = percent_rank_inc(contribution_values, product["contribution"])
        stock_coverage = coverage_values[idx]
        stock_pressure_percentile = percent_rank_inc(coverage_values, stock_coverage)
        priority_score = (
        ...
        discount = min(0.12, base_discount + (0.02 if stock_coverage >= 8 else 0.0))
        anchor_eligible = (
            movement_percentile >= 0.70
        ...
        candidate_eligible = (
            movement_band == "Low Movement"
            and product["available_stock"] > 0
            and stock_coverage >= 2
        )
        anchor_strength = 0.6 * movement_percentile + 0.4 * contribution_percentile
...
def update_scoring_sheet(ws, products):
    ...
    for row in range(2, MAX_PRODUCTS + 2):
        ...
        ws[f"N{row}"] = product["stock"] - product["reserved_stock"]
        ws[f"O{row}"] = (
            "OK" if abs(product["available_stock"] - (product["stock"] - product["reserved_stock"])) <= 1 else "CHECK"
        )
        ...
        ws[f"U{row}"] = "Yes" if product["anchor_eligible"] else "No"
        ws[f"V{row}"] = "Yes" if product["candidate_eligible"] else "No"
        ws[f"W{row}"] = product["discount"]
        ws[f"X{row}"] = f'{product["category"]}|anchor' if product["anchor_eligible"] else ""
        ws[f"Y{row}"] = product["name"] if product["anchor_eligible"] else ""
...
def fill_all_possible_bundles(ws, products):
    ...
    anchors = [p for p in products if p["anchor_eligible"]]
    candidates = [p for p in products if p["candidate_eligible"]]
    anchors.sort(key=lambda x: x["anchor_strength"], reverse=True)
    ...
    for anchor in anchors:
        for candidate in candidates:
            if anchor["product_id"] == candidate["product_id"]:
                continue
            score = 0.75 * candidate["priority_score"] + 0.25 * anchor["anchor_strength"]
            discount = candidate["discount"]
            category_mix = "Same Category" if anchor["category"] == candidate["category"] else "Cross Category"
            ws.append(
                [
                    f"B-{bundle_id:06d}",
                    2,
                    ...
                    score,
                    discount,
                    "Moves one slow mover using one customer-attracting anchor",
                ]
            )
            bundle_id += 1
    ...
    for anchor in anchors:
        for first, second in combinations(candidates, 2):
            if anchor["product_id"] in {first["product_id"], second["product_id"]}:
                continue
            score = (
            ...
            discount = max(first["discount"], second["discount"])
            same_category = (
                anchor["category"] == first["category"] == second["category"]
            )
            category_mix = "Same Category" if same_category else "Mixed Category"
            ws.append(
                [
                    f"B-{bundle_id:06d}",
                    3,
                    ...
                    score,
                    discount,
                    "Moves two slow movers with one anchor while keeping discount controlled",
                ]
            )
            bundle_id += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate all possible bundles (max 3 products) from workbook input data."
    )
    parser.add_argument(
        "workbook_path",
        nargs="?",
        default="bundle_planning_template.xlsx",
        help="Path to the workbook containing Input_Data sheet.",
    )
    ...
    if not products:
        raise ValueError("No products found in Input_Data. Please populate your data first.")

    score_products(products)
    scoring_ws = ensure_sheet(wb, SCORING_SHEET)
    update_scoring_sheet(scoring_ws, products)

    bundle_ws = ensure_sheet(wb, BUNDLE_SHEET)
    fill_all_possible_bundles(bundle_ws, products)

    wb.save(workbook_path)
    print(f"Updated workbook with all possible bundles: {workbook_path.resolve()}")
    print(f"Products: {len(products)}")
    print(f"Anchors: {sum(1 for p in products if p['anchor_eligible'])}")
    print(f"Slow-mover candidates: {sum(1 for p in products if p['candidate_eligible'])}")
    print(f"Bundle rows created: {bundle_ws.max_row - 1}")
